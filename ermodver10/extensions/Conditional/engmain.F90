...
!   wgtslf : weighting by the self-energy of the solute
!               The value of wgtslf is set internally from slttype and cltype
!               0 : no    soln
!                         refs without lattice-sum
!                         refs with lattice sum
!                              for flexbile solute and solute_LatticeSum = 1
!               1 : yes   refs with lattice sum
!                              for rigid solute or solute_LatticeSum = 0
...
!   solute_self_calc : calculation of the self-energy of the solute
!        0 : not calculated  1 : calculated
!
!  single-solute simulation parameters needed for self-energy calculation
!   solute_LatticeSum : use of Ewald, PME, or PPPM
!        0 : no use (default)  1 : used
!   solute_cut_ele : cutoff for electrostatic interaction
!   solute_cut_lwlj : lower limit of the LJ cutoff tapering function
!   solute_cut_uplj : upper limit of the LJ cutoff tapering function
!
...
!
   ...
   integer                                   :: numslt
   integer, dimension(:),        allocatable :: uvspec, sltlist
   integer, dimension(:),        allocatable :: uvmax, uvsoft
   ...
   real(kind=8) :: engnorm, engsmpl, avslf, voffset
   logical      :: voffset_initialized = .false.
   integer      :: solute_self_calc

   ! interaction in single-molecule simulation of the flexible solute
   !      needed for calculation of the self-energy correction
   !      effective only when slttype = SLT_REFS_FLEX
   integer :: solute_LatticeSum
   real    :: solute_cut_ele, solute_cut_lwlj, solute_cut_uplj
   ...
   integer :: OrderPrm_read = NO    ! not reading 'OrderPrmTraj'
   ...
   namelist /ene_param/ iseed, &
      ...
      slttype, wgtsys, wgtins, boxshp, estype, &
