   ...
   ! start of the extension for computing the conditional distributions
      if(do_conditional == YES) then
         ...
         if(myrank == 0) then
            ...
            if(slttype == SLT_SOLN) then
               do order_prmid = 1, order_size
                  cndnorm_tmp = cndnorm(order_prmid)
                  if(cndnorm_tmp > tiny) then
                     avuvcnd(division, :, order_prmid) = sluvcnd(:, order_prmid) / cndnorm_tmp
                  else
            ...
            do order_prmid = 1, order_size
               cndnorm_tmp = cndnorm(order_prmid)
               if(cndnorm_tmp > tiny) then
                  edcnd(:, order_prmid) = edcnd(:, order_prmid) / cndnorm_tmp
               else
            ...
            if(corrcal == YES) then
               ...
               do order_prmid = 1, order_size
                  cndnorm_tmp = cndnorm(order_prmid)
                  if(cndnorm_tmp > tiny) then
                     crcnd(:, :, order_prmid) = crcnd(:, :, order_prmid) / cndnorm_tmp
                  else
