...
!
   subroutine iniparam
      use engmain, only: init_params, &
         ...
         solute_self_calc, &
         solute_LatticeSum, solute_cut_ele, solute_cut_lwlj, solute_cut_uplj, &
      ...
      ! default setting for the single-molecule simulation of the solute
      solute_LatticeSum = NO       ! lattice-sum method not used
      ...
      select case(slttype)
       ...
       case(SLT_REFS_RIGID, SLT_REFS_FLEX)
         ...
         if((cltype == EL_EWALD) .or. (cltype == EL_PME) &
            ...
            wgtslf = YES           ! reweighting by the self-energy of solute
         endif
       case default
      ...
      ! single-molecule interaction of the solute
      select case(wgtslf)             ! solute self-energy correction
       case(NO)
         solute_self_calc = NO        ! self-energy not calculated
       case(YES)
         solute_self_calc = YES       ! self-energy calculated
       case default
         stop "Unknown wgtslf"
      end select

      ! lattice-sum method in the single-molecule simulation of the solute
      ! solute_LatticeSum can be set to YES only for refs with flexible solute
      if(slttype == SLT_REFS_FLEX) then
         select case(solute_LatticeSum)
          case(NO)        ! default
            solute_cut_ele  = large_distance    ! infinity in practice
            solute_cut_lwlj = large_distance    ! infinity in practice
            solute_cut_uplj = large_distance    ! infinity in practice
          case(YES)       ! lattice-sum method used for single solute
            ! solute_LatticeSum = 1 needs be stated explicitly in parameters_er
            if((cltype /= EL_EWALD) .and. (cltype /= EL_PME) &
               .and. (cltype /= EL_PPPM)) then  ! Ewald, PME, or PPPM not used
               stop "Lattice sum needs to be used at solute_LatticeSum = 1"
            endif
            ! same cutoff as in soln and refs used at default
            solute_cut_ele  = elecut
            solute_cut_lwlj = lwljcut
            solute_cut_uplj = upljcut
            ! solute_self_calc = YES at this point
            ! no self-energy correction at insertion for correct weighting
            wgtslf = NO
            ! no change of insertion orientation
            insorient = INSROT_NOCHANGE
         case default
            stop "Unknown solute_LatticeSum"
         end select
      endif
      ...
      if( (elecut <= 0.) .or. (lwljcut <= 0.) .or. (upljcut <= 0.) ) then
         stop "cutoff length must be positive"
      endif
      if(lwljcut > upljcut) stop "lwljcut <= upljcut must hold"

      ! check solute_self_calc parameter
      select case(solute_self_calc)
       case(NO)
         ! if solute_self_calc = NO, then wgtslf = NO
         ! if wgtslf = YES, then solute_self_calc = YES
         if(wgtslf == YES) then
            stop "Self-energy needs to be calculated when wgtslf = 1"
         endif
         ! if solute_self_calc = NO, then solute_LatticeSum = NO
         ! if solute_LatticeSum = YES, then solute_self_calc = YES
         if(solute_LatticeSum == YES) then
            stop "Self-energy needs to be calculated when solute_LatticeSum = 1"
         endif
       case(YES)
         if((cltype /= EL_EWALD) .and. (cltype /= EL_PME) &
            .and. (cltype /= EL_PPPM)) then  ! Ewald, PME, or PPPM not used
            stop "Lattice sum needs to be used at solute_self_calc = 1"
         endif
       case default
         stop "Unknown solute_self_calc"
      end select

      ! solute_LatticeSum = YES only when slttype = SLT_REFS_FLEX
      if(slttype /= SLT_REFS_FLEX) solute_LatticeSum = NO
      ...
      if((cltype == EL_EWALD) .or. (cltype == EL_PME) &
         .or. (cltype == EL_PPPM)) then  ! Ewald, PME, or PPPM
          if(boxshp == SYS_NONPERIODIC) call halt_with_error('set_prs')
      endif
