...
module engproc
   ...
   real(kind=8), dimension(:),   allocatable :: edovlp, uvovlp
   real(kind=8), dimension(:,:), allocatable :: avovlp
   real(kind=8) :: soln_total, soln_ovlp, norm_ovlp
...
contains
   ...
   subroutine enginit
      use engmain, only: numtype, nummol, engdiv, corrcal, selfcal, slttype, &
         moltype, sluvid, uvspec, numslt, sltlist, &
         ermax, numslv, uvmax, uvsoft, esmax, &
         uvcrd, edens, ecorr, escrd, eself, &
         voffset, &
         aveuv, slnuv, avediv, minuv, maxuv, &
      ...
      real(kind=8) :: factor, incre, cdrgvl(0:rglmax+1), ecpmrd(ermax_limit)
      ...
      !
      allocate( uvmax(numslv), uvsoft(numslv), ercrd(ermax_limit, 0:numslv) )
      !
...
3109        continue
         ...
         if(pemax > ermax_limit) call halt_with_error('eng_siz')
...
3209        continue
   ...
   subroutine engclear
      use engmain, only: corrcal, selfcal, slttype, SLT_SOLN, YES, &
         edens, ecorr, eself, slnuv, engnorm, engsmpl, avslf
      implicit none
      edens(:) = 0.0
...
912                     format(2i9, 9999f15.5)
   ...
   subroutine engstore(stnum)
      ...
      use engmain, only: maxcnf, skpcnf, engdiv, corrcal, selfcal, &
         ...
         edens, ecorr, eself, &
         aveuv, slnuv, avediv, minuv, maxuv, &
         engnorm, engsmpl, avslf, voffset, voffset_initialized, &
         wgtslf, solute_self_calc, &
...
#ifdef MPI
      ! synchronize voffset
      !    effective only at the first call since the value of voffset
      !    is common among all the proceses at the second and later calls
      if(solute_self_calc == YES .and. first_time) then
         ...
         ! if uninitialized, use voffset not to pollute results with NaN
      ...
      if(do_overlapped == YES) then
#ifdef MPI
         call mympi_reduce_real8_scalar(norm_ovlp, mpi_sum, 0)
         call mympi_reduce_real8_array(uvovlp, numslv, mpi_sum, 0)
         call mympi_reduce_real8_array(edovlp, ermax, mpi_sum, 0)
         if(division == engdiv) then
            call mympi_reduce_real8_scalar(soln_total, mpi_sum, 0)
            call mympi_reduce_real8_scalar(soln_ovlp, mpi_sum, 0)
         endif
#endif
         if(myrank == 0) then
            if(engdiv == 1) then
            ...
            write(eng_io, '(A2,A9,A15,A9,A21,A25)') "# ", "bin_left", "bin_middle", "spec", "ratio", "dens"
            do iduv = 1, ermax
            ...
            if(division == engdiv) then
               ...
               write(prb_io, '(a,f12.6)') ' Probability to find the system in the overlapped state = ', soln_ovlp / soln_total
               write(prb_io, '(a)') ' Free-energy change from the overlapped state to the solution state'
               write(prb_io, '(a,f12.6,a)') '                                                = ', temp * log(soln_ovlp / soln_total), ' kcal/mol'
               endfile(prb_io)
               close(prb_io)
      ...
      if(solute_self_calc == YES) then   ! calculation of solute self-energy
         avslf = engnorm / avslf
...
751            format(i5, 9999f15.5)
      ...
      if(selfcal == YES) then
         ...
         do iduv = 1, esmax
            middlebin = representative_bin_selfenergy(iduv)
            write(slf_io, '(g15.7,g25.15)') middlebin, eself(iduv)
         enddo
         endfile(slf_io)
         close(slf_io)
      endif
   ...
   subroutine get_uv_energy(stnum, stat_weight_solute, uvengy, out_of_range)
      ...
      ! all the coordinates in the system normalized (straightened)
      call realcal_prepare
      ! normalized coordinates deallocated at the end of update_histogram

      uvengy(:) = 0.
      uvrecp = 0.
      ! Calculate system-wide values in the case of Ewald, PME, or PPPM
      if((cltype == EL_EWALD) .or. (cltype == EL_PME) &
         .or. (cltype == EL_PPPM)) then
         call recpcal_prepare_solute(tagslt)
         call perf_time("rblk")
         ! real-space part of the solute-solvent pair energy
         call realcal_proc(tagslt, tagpt, slvmax, uvengy)
         call perf_time()
         call perf_time("kslf")
         ! reciprocal-space part of the solute self-energy
         uvrecp = recpcal_self_energy()
         call perf_time()
      endif

      ! self-energy of the solute
      ...
      if(current_solute_hash == solute_hash .or. &
         ...
         ! For refs calculation, the configuration of solute may change
         !   with random translation and/or rotation upon insertion,
         !   though the real-space part of the self-energy will not change.
         ! usreal is kept over subroutine calls during "save"
      else
         call perf_time("rslf")
         usreal = realcal_self(tagslt)          ! real-space part
         call perf_time()
      endif
      solute_hash = current_solute_hash
      residual = residual_ene(tagslt, tagslt)   ! residual part
      uvengy(0) = usreal + uvrecp + residual

      ! solute-solvent pair energy
      !$omp parallel do schedule(dynamic) private(i, k, pairep)
      do k = 1, slvmax
         i = tagpt(k)
         if(i == tagslt) cycle
         if((cltype == EL_EWALD) .or. (cltype == EL_PME) &
            .or. (cltype == EL_PPPM)) then      ! Ewald, PME, or PPPM
            ! sum of the reciprocal-space part and the residual part
            pairep = recpcal_energy(tagslt, i) + residual_ene(tagslt, i)
         else                                   ! bare Coulomb
            ! total (only real-space, no reciprocal-space and residual parts)
            pairep = realcal_bare(tagslt, i, 'inter')
         endif
         !$omp atomic
   ...
   subroutine update_histogram(stat_weight_solute, uvengy)
      use engmain, only: estype, slttype, corrcal, selfcal, ermax, &
         volume, temp, uvspec, &
         edens, ecorr, eself, &
         slnuv, minuv, maxuv, &
         stat_weight_system, &
         engnorm, engsmpl, avslf, voffset, voffset_initialized, &
         wgtslf, solute_self_calc, solute_LatticeSum, &
         elecut, lwljcut, upljcut, &
         solute_cut_ele, solute_cut_lwlj, solute_cut_uplj, &
         SLT_SOLN, SLT_REFS_RIGID, SLT_REFS_FLEX, &
         ES_NVT, ES_NPT, NO, YES
      use realcal, only: realcal_bare, realcal_cleanup
      use mpiproc
      implicit none
      real, intent(in) :: uvengy(0:slvmax), stat_weight_solute
      integer, allocatable :: insdst(:)
      integer :: i, k, q, iduv, iduvp, pti
      real :: self_E, pairep, total_weight, pot_syscut, pot_sglcut
      real :: elecut_tmp, lwljcut_tmp, upljcut_tmp
      real(kind=8) :: engnmfc
      ! very large distance
      ! huge(:) should not be used since the value could be further multiplied
      real, parameter :: large_distance = 1.0e5
      ! see subroutine iniparam for the use of large_distance

      total_weight = stat_weight_system * stat_weight_solute
      if(estype == ES_NPT) call volcorrect(total_weight)

      if(slttype == SLT_REFS_FLEX) then    ! insertion of flexible solute
         ! correction of the cutoff difference for solute intramolecular energy
         ! system cutoff temporarily stored
         elecut_tmp  = elecut
         lwljcut_tmp = lwljcut
         upljcut_tmp = upljcut
         ! without cutoff for electrostatics and with system cutoff for LJ
         if(solute_LatticeSum == YES) then
            elecut = -1.                   ! electrostatics not calculated
         else
            elecut = large_distance        ! no cutoff for electrostatic energy
         endif
         pot_syscut = realcal_bare(tagslt, tagslt, 'intra')
         ! temporarily set the cutoff to the value for single-solute simulation
         if(solute_LatticeSum == YES) then
            elecut = -1.                   ! electrostatics not calculated
         else
            ! default at (solute_LatticeSum = NO) = large_distance
            elecut = solute_cut_ele
         endif
         lwljcut = solute_cut_lwlj
         upljcut = solute_cut_uplj
         ! solute intramolecular energy with single-solute cutoff
         pot_sglcut = realcal_bare(tagslt, tagslt, 'intra')
         ! correction due to the cutoff difference of single-solute energy
         total_weight = total_weight * exp( - (pot_syscut - pot_sglcut) / temp)
         ! restore the system cutoff
         elecut  = elecut_tmp
         lwljcut = lwljcut_tmp
         upljcut = upljcut_tmp
      endif
      ...
      if(solute_self_calc == YES) then     ! calculation of solute self-energy
         ...
         self_E = uvengy(0) - voffset      ! shifted by offset
      endif

      select case(wgtslf)                  ! solute self-energy correction
       ! At wgtslf = NO, self_E is not used for reweighting
       case(NO)
         engnmfc = total_weight
         if(solute_self_calc == YES) then  ! averaging the negative self-energy
            avslf = avslf + exp(self_E / temp) * total_weight
         endif
       ! At wgtslf = YES, self_E is used for reweighting by self-energy
       case(YES)   ! Always solute_self_calc = YES when wgtslf = YES
         engnmfc = exp(- self_E / temp) * total_weight
         avslf = avslf + total_weight      ! normalization without self-energy
       case default
         stop "Unknown wgtslf"
      end select
      !
   ...
   function residual_ene(i, j) result(pairep)
      ...
      real :: pairep
      real :: epcl
      if(cltype == EL_COULOMB) then       ! bare Coulomb
         epcl = 0.
      else                                ! Ewald, PME, or PPPM
