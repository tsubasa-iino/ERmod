...
!$    npar = omp_get_max_threads()
   ...
   function realcal_bare(i, j, inttype) result(pairep)
      ...
      character(len=5), intent(in) :: inttype
      real :: pairep
      ...
      !
      select case(inttype)
       case('intra')       ! solute intramolecular interaction
         if(i /= j) stop "two particle arguments should be the same"
       case('inter')       ! solute-solvent intermolecular interaction
         if(i == j) stop "two particle arguments should not be the same"
         if(cltype /= EL_COULOMB) then
            stop "realcal_bare is called only when cltype is bare Coulomb"
         endif
       case default
         stop "Incorrect inttype in realcal_bare"
      end select
      ...
      if(boxshp == SYS_PERIODIC) then          ! when the system is periodic
         ...
         reelcut=huge(reelcut)
         half_cell(:) = 0.0                    ! suppress warnings
      endif
      if(elecut <= 0) reelcut = elecut         ! electrostatics not calculated
      ...
      do is = 1, ismax
         do js = 1, jsmax
            if((inttype == 'intra') .and. (js >= is)) cycle  ! intramolecular
            ati = specatm(is,i)
            atj = specatm(js,j)
            ljtype_i = ljtype(ati)
            ljtype_j = ljtype(atj)
...
!$    curp = omp_get_thread_num() + 1
   ...
   subroutine calc_gmx_switching_force_params(pow, lwljcut, upljcut, coeffA, coeffB, coeffC)
      ...
      if(upljcut <= lwljcut) then
         coeffA = 0.
         coeffB = 0.
         coeffC = 1.0 / (upljcut ** pow)
      else
         dfljcut = upljcut - lwljcut
         coeffA = - real(pow)                                           &
            * (real(pow + 4) * upljcut - real(pow + 1) * lwljcut)       &
            / ( (upljcut ** (pow + 2)) * (dfljcut ** 2) ) / 3.0
         coeffB = real(pow)                                             &
            * (real(pow + 3) * upljcut - real(pow + 1) * lwljcut)       &
            / ( (upljcut ** (pow + 2)) * (dfljcut ** 3) ) / 4.0
         coeffC = 1.0 / (upljcut ** pow)                                &
            - coeffA * (dfljcut ** 3) - coeffB * (dfljcut ** 4)
      endif
