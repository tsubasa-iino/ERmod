...
729         close(72)
   ...
   subroutine datread(cntrun)
      ...
      integer :: iduv, iduvp, i, k, m, pti, syscnt
      real :: factor, ampl, leftbin, centerbin
      ...
      ! FIXME: this part is kinda spaghetti and should be rewritten WITHOUT looping by syscnt!
      do syscnt = 1, 4
         if((syscnt == 2) .and. (slncor /= 'yes')) cycle
         if((syscnt >= 3) .and. (cntrun > 1) .and. (refmerge == 'yes')) cycle
         ...
         do i = ecmin, ecmax
            select case(clcond)
             case('basic', 'range')
               opnfile = engfile(syscnt)
             case('merge')
               suffnum = '.' // get_suffix(i, suffix_is_tt)
               select case(syscnt)
                case(1)               ! 1-D distribution in solution
                  opnfile = trim(solndirec) // '/' // trim(slndnspf)
                case(2)               ! 2-D distribution in solution
                  opnfile = trim(solndirec) // '/' // trim(slncorpf)
                case(3)               ! 1-D distribution in reference solvent
                  opnfile = trim(refsdirec) // '/' // trim(refdnspf)
                case(4)               ! 2-D distribution in reference solvent
                  opnfile = trim(refsdirec) // '/' // trim(refcorpf)
                case default
                  stop ' Bug in the program'
               end select
            ...
            select case(syscnt)
             case(1, 3)               ! 1-D distribution
               open(unit = 71, file = opnfile, status = 'old')
               read(71, *)            ! skip header
               k = 0
               m = 0
               do iduv = 1, ermax
                  read(71, *) leftbin, centerbin, pti, factor
                  if(pti /= k) then
                     k = pti
                     m = m + 1
                  endif
                  ...
                  select case(syscnt)
                   case(1)            ! solution
                     ...
                     rdcrd(iduv) = centerbin
                     rddst(iduv) =rddst(iduv) + wgtsln(i) * factor
                   case(3)            ! reference solvent
                     if( (bin_consistency_check(iduv) /= leftbin) &
                        .or. (rdcrd(iduv) /= centerbin)) then
                        stop "Solution and reference system energy coordinates are inconsitent"
                     endif
                     rddns(iduv) =rddns(iduv) + wgtref(i) * factor
                   case default
                     stop ' Bug in the program'
                  end select
               enddo
               close(71)
             case(2, 4)               ! 2-D correlation matrix
               ...
               select case(syscnt)
                case(2)               ! solution
                  rdslc(:, :) = rdslc(:, :) + wgtsln(i) * cormat_temp(:, :)
                case(4)               ! reference solvent
                  rdcor(:, :) = rdcor(:, :) + wgtref(i) * cormat_temp(:, :)
                case default
                  stop ' Bug in the program'
               end select
...
contains
   ...
   subroutine chmpot(prmcnt, cntrun)
      ...
      integer :: group, inft
      integer :: iduv, iduvp, pti, cnt, j, k, m, systype, cntdiv, ge_perslv
      ...
      do systype = 1, 2
         do iduv = 1, ermax
            ...
            select case(systype)
             case(1)                  ! solution
               edist(k) = edist(k) + rddst(iduv)
             case(2)                  ! reference solvent
               edens(k) = edens(k) + rddns(iduv)
             case default
               stop ' Bug in the program'
            end select
         end do
         if((systype == 2) .or. (slncor == 'yes')) then
            do iduv = 1, ermax
               do iduvp = 1, ermax
                  k = idrduv(iduv)
                  m = idrduv(iduvp)
                  select case(systype)
                   case(1)            ! solution
                     edscr(m,k) = edscr(m,k) + rdslc(iduvp, iduv)
                   case(2)            ! reference solvent
                     ecorr(m,k) = ecorr(m,k) + rdcor(iduvp, iduv)
                   case default
                     stop ' Bug in the program'
                  end select
               end do
      ...
      do pti = 1, numslv
         ...
         do iduv = 1, gemax
            if(uvspec(iduv) == pti) then
               ...
               ! approximate integration of indirect PMF over coupling parameter
               lcsln = intgcv(slncv(iduv), 1)   ! solution
               lcref = intgcv(inscv(iduv), 2)   ! reference solvent
               if((slncor == 'yes') .and. (edist(iduv) > soln_zero) &
                  .and. (edens(iduv) <= refs_zero)) then
                  ! special case to be examined and fixed later
                  lcsln = intgcv(sdrcv(iduv) + zrsdr(pti), 3)
               endif
...
383      format('  Zero shift for solution correlation = ', 9999f12.4)
   ...
   subroutine getinscv
      ...
      implicit none
      integer :: iduv, iduvp, pti, systype, k, inv_info
      ...
      do systype = 1, 2       ! 1: solution   2: reference solvent
         if((systype == 1) .and. (slncor /= 'yes')) cycle
         invmtrx_sys = invmtrx
         ...
         if((invmtrx_sys == 'gce') .or. (invmtrx_sys == 'reg')) then
            allocate( edmcr_invertible(gemax, gemax) )
            edmcr_invertible(:,:) = edmcr(:,:)
            work(:) = ddiff(:)
            where(edvec(:) <= zero) work(:) = 0.0

            if(invmtrx_sys == 'gce') then  ! grand canonical ensemble
               ...
               do pti = 1, numslv
                  k = ecorigin(pti, systype)   ! coordinate at energy origin
            ...
            if(inv_info == 0) then
               ...
               invmtrx_sys = 'evd'         ! alternative solution with evd
            endif
            deallocate( edmcr_invertible )
         endif

         if(invmtrx_sys == 'evd') then     ! eigenvalue decomposition
         ...
         do pti = 1, numslv
            select case(zerosft)
             case('eczr')
               k = ecorigin(pti, systype)  ! coordinate at energy = 0
               if(systype == 1) then       ! solution
                  cvzero = sdrcv(k)
               else                        ! reference solvent
                  cvzero = inscv(k)
             ...
             case('mxco')
               cvzero = cvfcen(pti, systype, 'inscv', 'smpl', 'yes')
             case default
               cvzero = 0.0
            end select
            zerouv(pti) = cvzero
         ...
         do pti = 1, numslv
            select case(zerosft)
             ...
             case('mxco')
               factor = wgtmxco(pti)
               cvzero = factor * cvfcen(pti, systype, 'inscv', wgtfnform, 'not') &
                      - (1.0 - factor) * zerouv(pti)
             case('zero')
               cvzero = cvfcen(pti, systype, 'inscv', wgtfnform, 'yes')
             case('cntr')
               cvzero = cvfcen(pti, systype, 'inscv', wgtfnform, 'not')
             case default
               stop ' zerosft not properly set '
            end select
            do iduv = 1, gemax
   ...
   !
   real function cvfcen(pti, systype, cvftype, wgttype, engtype)
      implicit none
      ! pti : identifier of solvent species
      ! systype = 1: solution   systype = 2: reference solvent
      integer, intent(in) :: pti, systype
      character(len=5), intent(in) :: cvftype
      ...
      call getwght(weight, pti, systype, cvftype, wgttype, engtype)
      factor = 0.0
      errtag = .false.
      do iduv = 1, gemax
         if(uvspec(iduv) == pti) then
            select case(cvftype)
             case('slncv')
               if(systype == 1) cvfnc = slncv(iduv)
               if(systype == 2) errtag = .true.
             case('inscv')
               if(systype == 1) cvfnc = sdrcv(iduv)
               if(systype == 2) cvfnc = inscv(iduv)
             case('uvcrd')
               cvfnc = uvcrd(iduv)
             case default
               errtag = .true.
   ...
   !
   subroutine getwght(weight, pti, systype, cvftype, wgttype, engtype)
      implicit none
      ...
      do iduv = 1, gemax
         if(uvspec(iduv) == pti) then
            weight(iduv) = wgtdst(iduv, systype, cvftype, wgttype)
         endif
      end do
      if(engtype == 'yes') then
   ...
   !
   integer function ecorigin(pti, systype)
      implicit none
      ! pti : identifier of solvent species
      ! systype = 1: solution   systype = 2: reference solvent
      integer, intent(in) :: pti, systype
      ...
      ecorigin = k
      return
   end function ecorigin
   !
   !
   real function wgtdst(iduv, systype, cvftype, wgttype)
      use sysvars, only: wgtf2smpl
      implicit none
      ! systype = 1: solution   systype = 2: reference solvent
      integer, intent(in) :: iduv, systype
      character(len=5), intent(in) :: cvftype
      character(len=4), intent(in) :: wgttype
      ...
      jdg = 1
      if(systype == 2) then
         if(cvftype == 'slncv')  jdg = 9
         if(cvftype == 'extsl')  jdg = 9
   ...
   !
   !
   real function intgcv(indpmf, systype)
      use sysvars, only: functional, thnc_cutoff
      implicit none
      ...
      if(first_time) then
         ...
         select case(functional)
          case('pyhnc', 'PYHNC', 'py-hnc', 'PY-HNC')
            functional = 'pyhnc'
          case('thnc', 'tHNC', 'THNC')
            functional = 'thnc'
            if(thnc_cutoff >= - tiny) then
               write(6, "(A,f12.4,A)") "  Check if the input cutoff of ", &
                  thnc_cutoff, " kcal/mol is correct for tHNC"
               write(6, *)
            endif
          case('hnc', 'HNC')
            functional = 'thnc'            ! using the routine for tHNC
            thnc_cutoff = -huge(indpmf)    ! minus infinity
          case default
            functional = 'pyhnc'           ! default functional = 'pyhnc'
         end select
         first_time = .false.
      endif

      if((systype == 3) .and. (functional /= 'pyhnc')) then
         stop "Correlation matrix can be used in solution only when functional = pyhnc"
      endif
      ...
      select case(functional)
       case('pyhnc')
         select case(systype)
          case(1)         ! solution
            if(scaledcv < - zero) then     ! PY
               intg = 1.0 + scaledcv + scaledcv / (exp(- scaledcv) - 1.0)
            else                           ! HNC
               intg = scaledcv / 2.0
            endif
          case(2)         ! reference solvent
            if(scaledcv < - zero) then     ! PY
               intg = 1.0 + log(1.0 - scaledcv) * (1.0 / scaledcv - 1.0)
            else                           ! HNC
               intg = scaledcv / 2.0
            endif
          case(3)         ! special case to be examined further
            if(scaledcv > zero) then       ! PY
               intg = 1.0 - log(1.0 + scaledcv) * (1.0 / scaledcv + 1.0)
            else                           ! HNC
               intg = - scaledcv / 2.0
          ...
          case default
            stop "Incorrct systype argument in intgcv"
         end select

       case('thnc')
         select case(systype)
          case(1)         ! solution
            intg = scaledcv / 2.0          ! HNC
          case(2)         ! reference solvent
            if(indpmf < thnc_cutoff) then    ! indpmf used, instead of scaledcv
               intg = 0.0                  ! truncated
            else
               intg = scaledcv / 2.0       ! HNC
            endif
   ...
   subroutine distnorm
      implicit none
      integer :: iduv, iduvp, pti, systype, itrcnt
   ...
   subroutine distshow
      implicit none
      integer :: iduv, pti, systype, ecmin, ecmax, k, ilist(gemax)
      real :: factor, ratio
      ...
      do pti = 1, numslv
         do systype = 1, 2       ! 1: solution   2: reference solvent
            ...
            do iduv = ecmin, ecmax
               if((systype == 2) .and. (edhst(iduv) <= zero)) then
                  write(6, "(A,i5,A,g14.6)") "     No sampling at ", iduv, &
                     " with energy ", uvcrd(iduv)
               endif
            end do

            if(systype == 1) write(6, "(A,g12.4)") "     Nonzero component ratio in solution  = ", ratio
            if(systype == 2) write(6, "(A,g12.4)") "     Nonzero component ratio at insertion = ", ratio
